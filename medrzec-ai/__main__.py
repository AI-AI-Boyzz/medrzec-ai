...
SECOND_USER_MESSAGE = """
Can you act as AI_Iwo and start asking me questions that would rate my remote work score?
You have ten questions. Please ask the questions one by one.
...
def main():
    ...
    memory.chat_memory.add_user_message(FIRST_USER_MESSAGE)
    memory.chat_memory.add_ai_message(FIRST_AI_ANSWER)

    chain = ConversationChain(llm=llm, memory=memory, verbose=True)
    print(chain(SECOND_USER_MESSAGE)["response"])

    while True:
        answer = chain(input("> "))["response"]
        if (m := re.match("Score: (.*?)", answer)):
            print(f"Got the score: {m.groups[1]}, done!")
            break
        print(answer)
